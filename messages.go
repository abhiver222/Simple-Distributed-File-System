...
func sendSyn() {
	for {
		...
		if N >= MIN_HOSTS {
			msg := message{getIP(), "SYN", time.Now().Format(time.RFC850), file_info{"", nil, 0}}
...
func sendAck(host string) {
	msg := message{currHost, "ACK", time.Now().Format(time.RFC850), file_info{"", nil, 0}}
...
func connectToIntroducer() {
	msg := message{currHost, "Joining", time.Now().Format(time.RFC850), file_info{"", nil, 0}}
	var targetHosts = make([]string, 1)
...
func leaveGroup() {
	msg := message{currHost, "Adios", time.Now().Format(time.RFC850), file_info{"", nil, 0}}
...
func yup() {
	msg := message{currHost, "yup", time.Now().Format(time.RFC850), file_info{"", nil, 0}}
	var targetHosts = make([]string, 1)
	targetHosts[0] = INTRODUCER
